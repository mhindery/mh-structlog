...
# from logging import INFO, DEBUG, ERROR, WARN, WARNING, CRITICAL, FATAL  # noqa: F401


class StructlogLoggingConfigException(Exception):
...
def _add_flattened_extra(_, __, event_dict):
    ...
    if event_dict.get("_from_structlog", False):
        ...
        extra = event_dict.pop("extra", dict())
...
def setup(
    ...
    if log_format not in ['console', 'aws_json']:
        raise StructlogLoggingConfigException("Unknown logging format requested.")
    ...
    if logging_configs:
        for lc in logging_configs:
            for k, v in lc.get("loggers", dict()).items():
                if k in ['', 'root']:
                    raise StructlogLoggingConfigException(
            ...
            for k, v in lc.get("handlers", dict()).items():
            ...
            for k, v in lc.get("formatters", dict()).items():
            ...
            for k, v in lc.get("filters", dict()).items():
